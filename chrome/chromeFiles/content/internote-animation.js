...
var internoteAnimation = {
	...
	{
		if(this.stickiesGetAnimation(container))
	    {
	    	var newNote = new this.stickiesNoteSettings(parseInt(container.style.top, 10),parseInt(container.style.left, 10),parseInt(container.style.width, 10),parseInt(container.style.height, 10));
	        var newAnim = new this.stickiesNoteAnimation(0, container, newNote);
	        
       		newAnim.animEq = "var y; if(x>=0){ y = (1.0 / (1 + Math.exp((-x*12)+6))); } else { y = (1.0 / (1 + Math.exp((x*12)+6))); } y *= animation.target.targetHeight;";
       		newAnim.animValue = 1;
       		newAnim.animStart = 1;
       		newAnim.animEnd = -1;
       		newAnim.animSkip = -.025;
       		//newAnim.animEq = "var y = (x) * animation.target.targetHeight";
			newAnim.animSet = "animation.animEl.style.width = y + 'px'; animation.animEl.style.left = (animation.target.targetLeft + (animation.target.targetWidth - y)/2) + 'px';";
			newAnim.animClean = "animation.animEl.style.width = animation.target.targetWidth + 'px'; animation.animEl.style.left = animation.target.targetLeft + 'px';";
			newAnim.animComp = "var comp = (animation.animValue <= animation.animEnd)";
